...
class DevPackageHandler extends \Zhiyi\Plus\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.plus-illusts.migrations')
        );
    ...
    {
        $path = $this->app->make('path.plus-illusts.seeds');
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-illusts.resources').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.plus-illusts.resources').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
